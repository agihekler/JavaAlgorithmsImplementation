...
public class Sorts {
    ...
    public void testBubbleSorts() {
        ...
        // test case added to check when empty array is passed
        Integer[] empty = {};
        result = BubbleSort.sort(empty.clone());
        assertTrue("Bubble sort empty error. result="+print(result), check(result));
        // test case added to check when single element is passed
        Integer[] singleelement = {25};
        result = BubbleSort.sort(singleelement.clone());
        assertTrue("Bubble sort single element error. result="+print(result), check(result));
        // test case added to check when array contains duplicates
        Integer[] duplicates = {2, 2, 5, 10, 4};
        result = BubbleSort.sort(duplicates.clone());
        assertTrue("Bubble sort duplicate elements error. result="+print(result), check(result));
